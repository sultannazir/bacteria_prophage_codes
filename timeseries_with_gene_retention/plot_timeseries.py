...
N = 10 # Number of iterations goes from 1 to N
...
for pi in range(7):
    for vidx in range(3):
        for n in range(0,N):
            dat = pd.read_csv('data/pi'+str(pi+11)+'VD'+str(VDs[vidx])+'iter'+str(n+1)+'IwGR1timeseries.dat', delimiter='\t', names=['time', 'U', 'Uc', 'L', 'Lc', 'Lp', 'Lcp', 'V', 'VT'])
            dat['fracP'] = (dat['Lp']+dat['Lcp'])/(dat['Uc']+dat['Lc']+dat['Lp']+2*dat['Lcp'])
            axes[pi][vidx].plot(dat['fracP'])
            axes[pi][vidx].set_ylim(-0.1,1.1)
    aa = axes[pi][-1].twinx()
    aa.set_yticks([])
    aa.set_ylabel(r"$\log_{10} \pi = $"+str(-(pi+11)/5), rotation=0, labelpad = 40)
...
#plt.show()
plt.savefig("timeseries_IwGR.png")
